...
public class TeleporterVoid extends Teleporter
{
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_)
	{
			...
			for (int l = -128; l <= 128; ++l)
			{
				...
				for (int i1 = -128; i1 <= 128; ++i1)
				{
					for (BlockPos blockpos = blockpos4.add(l, worldObj.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1)
					{
						...
						if (worldObj.getBlockState(blockpos).getBlock() == TheVoid.void_portal)
						{
							while (worldObj.getBlockState(blockpos1 = blockpos.down()).getBlock() == TheVoid.void_portal)
							{
								blockpos = blockpos1;
							}
		...
		if (d0 >= 0.0D)
		{
			if (flag1)
			{
				coordCache.add(k, new PortalPosition((BlockPos)object, worldObj.getTotalWorldTime()));
			...
			if (worldObj.getBlockState(((BlockPos)object).west()).getBlock() == TheVoid.void_portal)
			{
				enumfacing = EnumFacing.NORTH;
			}

			if (worldObj.getBlockState(((BlockPos)object).east()).getBlock() == TheVoid.void_portal)
			{
				enumfacing = EnumFacing.SOUTH;
			}

			if (worldObj.getBlockState(((BlockPos)object).north()).getBlock() == TheVoid.void_portal)
			{
				enumfacing = EnumFacing.EAST;
			}

			if (worldObj.getBlockState(((BlockPos)object).south()).getBlock() == TheVoid.void_portal)
			{
			...
			for (i3 = -1; i3 <= 1; ++i3)
			{
				for (j3 = 1; j3 < 3; ++j3)
				{
					for (k3 = -1; k3 < 3; ++k3)
					{
						...
						worldObj.setBlockState(new BlockPos(l3, i4, j4), flag ? Blocks.end_stone.getDefaultState() : Blocks.air.getDefaultState());
					}
				}
			}
		}

		IBlockState blockstate = TheVoid.void_portal.getDefaultState().withProperty(BlockPortal.AXIS, l5 != 0 ? EnumFacing.Axis.X : EnumFacing.Axis.Z);
		...
		for (j3 = 0; j3 < 4; ++j3)
		{
			for (k3 = 0; k3 < 4; ++k3)
			{
				for (l3 = -1; l3 < 4; ++l3)
				{
					...
					worldObj.setBlockState(new BlockPos(i4, j4, k4), flag1 ? Blocks.end_stone.getDefaultState() : blockstate, 2);
				}
			}
